...
def get_internal_links(bs_obj, include_url):
    ...
    for link in bs_obj.find_all('a', href=re.compile(f'^(/|.*){include_url})')):
...
def get_external_links(bs_obj, exclude_url):
    ...
    for link in bs_obj.find_all('a', href=re.compile(f'^(http|www)((?!{exclude_url}).)*$')):
        if link.attrs['href'] is not None:
...
follow_external_only('https://xvideos.com')
